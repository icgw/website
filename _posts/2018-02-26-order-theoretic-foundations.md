...
# $$\lozenge$$ 完全晶格 $$\lozenge$$

## 晶格、完全晶格、单位元、零元

1. 如果偏序集 **V** := $$(V, \leq)$$ 中任意两个元素存在上确界和下确界，则称其为晶格；
2. 如果 $$V$$ 的所有子集 $$X$$ 存在上确界和下确界，则称其为完全晶格；
3. 晶格 **V** 的最大元素称为单位元，记为 $$1_V$$；最小元素称为零元，记为 $$0_V$$。

> 特别约定， $$\varnothing$$ 的下确界为 $$1_V$$，上确界为 $$0_V$$，即 $$\bigwedge\varnothing = 1_V, \bigvee\varnothing = 0_V$$ 。

显然，所有非空有限晶格都是完全晶格。

> **给定偏序集 $$(M, \leq)$$，其每个子集的下确界都存在，则它是完全晶格。**
>
...
# 几个晶格的例子
1. 对于有限集合 $$M$$，$$(\mathcal{P}(M), \subseteq)$$ 是完全晶格；
2. 对于实数区间 $$[a, b]$$，$$([a, b], \leq)$$ 是完全晶格；
3. $$(\Re, \leq)$$ 是晶格，但不是完全的，因为$$\Re$$上没有最大元素，也没有最小元素。
...
> **有限晶格的一个元素 $$v$$ 是 $$\bigvee$$-不可约的 $$\Leftrightarrow$$ 它有且仅有一个下近邻。**
>
> **证明：**
>
> $$v$$ 是$$\bigvee$$-不可约的，即 $$v \neq v_*$$
...
> 有限晶格 **V**，集合 $$J$$(**V**) 是上确界稠密的，集合 $$M$$(**V**)是下确界稠密的。
>
...
1. 零元的上近邻，称为原子；
2. 单位元的下近邻，称为反原子；
3. 完全晶格的所有元素都可以由其原子某种组合的并得到，则称其是原子态的。

![atom_ex](../images/lattice_atom_ex.png){:width="66%"}

上图实线方框内的元素是原子，虚线方框内的元素是反原子，并且该完全晶格是原子态的。
